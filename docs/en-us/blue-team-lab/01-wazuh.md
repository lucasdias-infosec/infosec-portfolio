...
A single-node (all-in-one) installation mode was selected, in which:

Wazuh Indexer
Wazuh Server (Manager)
Wazuh Dashboard

operate on the same virtual machine.
...
Local Monitoring (Internal Agent)
...
The confirmed status was Active, indicating that local monitoring is operational.
